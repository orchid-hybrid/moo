...
(define (mut-conv mvars replace e)
  (cond ((symbol? e) (if (member e replace) `(cell-value ,e) e))
        ((primitive-value? e) e)
        ((pattern? '(if _ _ _) e)
         (mut-collect mvars e)
         (let ((b (cadr e)) (thn (caddr e)) (els (cadddr e)))
        ...
        ((pattern? '(lambda _ _) e)
         (let ((vs (if (list? (cadr e)) (cadr e) (list (cadr e))))
           ...
           (let ((inner-mut-vars (make-cell '())))
             (mut-collect inner-mut-vars  m)
             `(lambda ,vs
                (lambda ,vs ,(mut-conv inner-mut-vars (append replace (cell-value inner-mut-vars)) m))
                ,@(map (lambda (v) (if (member v (cell-value inner-mut-vars))
                                      `(cons ,v '())
                                      v))
                       vs)))))
        ((pattern? '(set! _ _) e)
...
(define (compile form)
  (let ((bound-variables '()))
    ...
    (let ((cps-form (T-c form 'halt)))
      ...
      (let ((mut-form (mut-conv (make-cell '()) '() cps-form)))
        (display 'mut) (newline)
        (pretty-print mut-form) (newline)
        (let ((cc-form (closure-convert bound-variables bound-variables (make-cell '()) mut-form)))
          (display 'cc) (newline)
