...
(define (emit-c exp)
  (cond ((equal? exp 'null) ((formatter (list "NULL")) '()))
        ((equal? exp 'env) ((formatter (list "self->val.closure.environment"))'()))
        ((symbol? exp) ((formatter (list ~m)) (list exp)))
        ((number? exp) ((formatter (list ~a)) (list exp)))
        ((null? exp) ((formatter (list "(scm){ .typ=scm_type_null }"))'()))
        ((boolean? exp) ((formatter (list "bool(" ~a ")")) (list (if exp 1 0))))
        ((string? exp) ((formatter (list ~s)) (list exp)))
        ((pattern? '(* _) exp)
         ((formatter (list "*" ~e)) (list (cadr exp))))
        ((pattern? '(* _ _) exp)
         ((formatter (list ~e "*" ~e)) (list (cadr exp) (caddr exp))))
        ...
        ((pattern? '(if _ _ _) exp)
         ((formatter (list "if (" ~a "(" ~e ")) {" ~% (~@ (list ~e ~%)) "} else {" ~% (~@ (list ~e ~%)) "}" ~%))
          (list (if (symbol? (cadr exp)) "scm_truepstar" "scm_truep")
                (cadr exp) (caddr exp) (cadddr exp))))
        ((pattern? '(pop) exp)
         ((formatter (list "stack_pop()")) '()))
        ((pattern? '(push _) exp)
         ((formatter (list "stack_push(" ~e ");")) (list (cadr exp))))
        ((pattern? '(closure _ _ _) exp)
         ((formatter (list "closure(" ~e ", " ~e ", " ~e ")"))
          (list (cadr exp) (caddr exp) (cadddr exp))))
        ((pattern? '(vector-ref _ _) exp)
         ((formatter (list ~e "[" ~e "]")) (list (cadr exp) (caddr exp))))
        ((pattern? '(ref _ _) exp)
         ((formatter (list ~e "[" ~e "]")) (list (cadr exp) (caddr exp))))
        ((pattern? '(sizeof _) exp)
         ((formatter (list "sizeof(" ~a ")")) (list (cadr exp))))
        ((pattern? '(gc-alloc* _) exp)
         ((formatter (list "gc_alloc(" ~e "*sizeof(scm))")) (list (cadr exp))))
        ((pattern? '(gc-alloc*+* _) exp)
         ((formatter (list "gc_alloc(" ~e "*sizeof(scm*) + " ~e "*sizeof(scm))")) (list (cadr exp) (cadr exp))))
        ((pattern? '(gc-alloc-scm _) exp)
         ((formatter (list "gc_alloc_scm(" ~e ")")) (list (cadr exp))))
        ((pattern? '(make-nil) exp)
         ((formatter (list "(scm){ .typ=scm_type_null }")) '()))
        ((pattern? '(make-symbol _) exp)
         ((formatter (list "sym(" ~e ")")) (list (cadr exp))))
        ((pattern? '(make-string _) exp)
         ((formatter (list "str_alloc(" ~e ")")) (list (cadr exp))))
        ((pattern? '(make-number _) exp)
         ((formatter (list "num(" ~e ")")) (list (cadr exp))))
        ((pattern? '(set! _ _) exp)
         ((formatter (list ~e " = " ~e ";")) (list (cadr exp) (caddr exp))))
        ((pattern? '(+= _ _) exp)
         ((formatter (list ~e " += " ~e ";")) (list (cadr exp) (caddr exp))))
        ((pattern? '(declare _ _ (hold (pop))) exp)
         ((formatter (list ~a " " ~e " = nursery_hold(" ~e ");")) (list (cadr exp) (caddr exp) (cadr (cadddr exp)))))
        ((pattern? '(declare _ _ _) exp)
         ((formatter (list ~a " " ~e " = " ~e ";"))
          (list (cadr exp) (caddr exp) (cadddr exp))))
